...
| --- | ------------------- |
| Windows | Create `C:\mos` folder. Right-click on this [mos.exe](https://mongoose-os.com/downloads/mos-release/win/mos.exe) link,  choose "Save target as", save `mos.exe` into the `C:\mos` folder. Then, double-click on saved executable to start Web UI, or start a command line prompt and type: <pre>cd C:\mos<br>mos --help</pre> |
